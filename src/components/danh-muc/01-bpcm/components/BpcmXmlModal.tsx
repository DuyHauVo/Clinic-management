    ...
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center z-50 p-4 ant-modal-anim">
      <div className="bg-white rounded-2xl max-w-4xl w-full max-h-[90vh] flex flex-col shadow-2xl overflow-hidden border border-slate-200">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 mt-0.5">
                Dữ liệu hiện hành • {items.length} bản ghi BPCM
              </p>
            </div>
          </div>
          <button
        ...
        <div className="p-6 overflow-y-auto flex-1 space-y-4">
          {items.length === 0 ? (
            <div className="p-8 text-center bg-slate-50 rounded-2xl border border-dashed border-slate-300 space-y-3">
              ...
              <p className="text-sm font-bold text-slate-800">Chưa có dữ liệu Bộ Phận Chuyên Môn</p>
            ...
            <>
              ...
              <div className="flex items-center gap-2 border-b border-slate-200 pb-3">
                ...
                >
                  <FileCode size={14} />
                  <span>XML Gốc (&lt;DANHSACH_BOPHAN_KBCB&gt;)</span>
                </button>
                ...
                >
                  <Send size={14} />
                  <span>Đặc Tả API Gửi Cổng EGW (GuiDanhMuc01)</span>
                </button>
              </div>
              ...
              {tab === 'xml' && (
                <div className="space-y-2">
                  <div className="flex items-center justify-between text-xs text-slate-500">
                    <span>Định dạng XML chuẩn BHXH (Loại 70)</span>
                    <button
                      type="button"
                      onClick={() => onCopy(xmlContent)}
                      className="text-[#1677ff] font-bold hover:underline flex items-center gap-1"
                    >
              ...
              {tab === 'base64' && (
                <div className="space-y-2">
                  <div className="flex items-center justify-between text-xs text-slate-500">
                    <span>Chuỗi Base64 truyền vào tham số <code className="text-[#1677ff] font-mono">fileHsBase64</code></span>
                    <button
                      type="button"
                  ...
                  <textarea
                    readOnly
                    className="w-full p-4 bg-slate-900 text-blue-300 rounded-xl text-[11px] font-mono h-[300px] border border-slate-800 leading-relaxed outline-none select-all resize-none"
                    value={base64Content}
                  />
                </div>
              )}
              ...
              {tab === 'api' && (
                <div className="space-y-4 text-xs">
                  <div className="p-4 bg-slate-900 text-slate-200 rounded-xl space-y-3 font-mono text-[11px]">
                    <div className="flex items-center justify-between border-b border-slate-800 pb-2">
                      <span className="text-emerald-400 font-bold">
                        POST https://egw.baohiemxahoi.gov.vn/api/DanhMucGW/GuiDanhMuc01_BPCMKBCB
                      </span>
                  ...
                  <div className="p-4 bg-emerald-50 rounded-xl border border-emerald-200 flex items-center justify-between">
                    <div>
                      <div className="flex items-center gap-2">
                        <h5 className="font-bold text-emerald-900">Gửi Thử Nghiệm Mô Phỏng Cổng BHXH</h5>
                        <span className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-emerald-100 text-emerald-800">Demo</span>
                      </div>
                      <p className="text-emerald-700 text-[11px] mt-0.5">
                        Gói tin chứa {items.length} bản ghi BPCM sẽ được mã hóa Base64 gửi mô phỏng kiểm tra cấu trúc.
                      </p>
                    </div>
                    <button
                      type="button"
                  ...
                  {apiResponse && (
                    <div className="p-4 bg-slate-900 text-emerald-400 rounded-xl font-mono text-[11px] space-y-1">
                      <div className="text-slate-400 font-bold border-b border-slate-800 pb-1">KẾT QUẢ PHẢN HỒI (HTTP 200 OK):</div>
        ...
        <div className="p-4 border-t border-slate-200 bg-slate-50 flex items-center justify-between">
          <button
            type="button"
            onClick={() => onCopy(tab === 'xml' ? xmlContent : base64Content)}
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              type="button"
              onClick={onClose}
              className="px-4 py-2 bg-[#1677ff] hover:bg-blue-600 text-white rounded-xl text-xs font-bold"
            >
              Đóng
            </button>
          </div>
