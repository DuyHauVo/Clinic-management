...
interface ThietBiXmlModalProps {
  isOpen: boolean;
  onClose: () => void;
  items: DmThietBiItem[];
  xmlContent: string;
      ...
      <div className="bg-white rounded-2xl max-w-4xl w-full max-h-[90vh] flex flex-col shadow-2xl overflow-hidden border border-slate-200">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h3 className="text-base font-extrabold text-slate-900">
                  Cấu Trúc XML & Chuỗi Base64 Ký Số Mẫu 04/DM
                </h3>
                <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-cyan-100 text-cyan-800 border border-cyan-200">
                  Loại HS 11 - GuiDanhMuc04_DMVTYT
                </span>
              </div>
              <p className="text-xs text-slate-500 mt-0.5">
                Dữ liệu hiện hành • {items.length} thiết bị / vật tư y tế
              </p>
            </div>
          </div>
          <button
        ...
        <div className="p-6 overflow-y-auto flex-1 space-y-4">
          {items.length === 0 ? (
            <div className="p-8 text-center bg-slate-50 rounded-2xl border border-dashed border-slate-300 space-y-3">
              ...
              <p className="text-sm font-bold text-slate-800">Chưa có dữ liệu Thiết Bị Y Tế</p>
            ...
            <>
              ...
              <div className="flex items-center gap-2 border-b border-slate-200 pb-3">
                ...
                >
                  <Send size={14} />
                  <span>Đặc Tả API Cổng EGW (GuiDanhMuc04_DMVTYT)</span>
                </button>
              </div>
              ...
              {tab === 'xml' && (
                <div className="space-y-2">
                  <div className="flex items-center justify-between text-xs text-slate-500">
                    <span>Định dạng XML chuẩn Bộ Y tế & BHXH (Loại HS 11)</span>
                    <button
                      type="button"
              ...
              {tab === 'base64' && (
                <div className="space-y-2">
                  <div className="flex items-center justify-between text-xs text-slate-500">
                    <span>Chuỗi Base64 ký số truyền vào tham số <code className="text-cyan-600 font-mono">fileHsBase64</code></span>
                    <button
                      type="button"
                  ...
                  <textarea
                    readOnly
                    className="w-full p-4 bg-slate-900 text-cyan-300 rounded-xl text-[11px] font-mono h-[300px] border border-slate-800 leading-relaxed outline-none select-all resize-none"
                    value={base64Content}
                  />
                </div>
              )}
              ...
              {tab === 'api' && (
                <div className="space-y-4 text-xs">
                  <div className="p-4 bg-slate-900 text-slate-200 rounded-xl space-y-3 font-mono text-[11px]">
                    <div className="flex items-center justify-between border-b border-slate-800 pb-2">
                      <span className="text-cyan-400 font-bold">
                        POST https://egw.baohiemxahoi.gov.vn/api/DanhMucGW/GuiDanhMuc04_DMVTYT
                      </span>
                  ...
                  <div className="p-4 bg-cyan-50 rounded-xl border border-cyan-200 flex items-center justify-between">
                    <div>
                      <div className="flex items-center gap-2">
                        <h5 className="font-bold text-cyan-900">Gửi Thử Nghiệm Mô Phỏng Cổng BHXH</h5>
                        <span className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-cyan-100 text-cyan-800">Demo</span>
                      </div>
                      <p className="text-cyan-700 text-[11px] mt-0.5">
                        Gói tin chứa {items.length} thiết bị / vật tư y tế sẽ được đóng gói mã hóa Base64 gửi mô phỏng.
                      </p>
                    </div>
                    <button
                      type="button"
        ...
        <div className="p-4 border-t border-slate-200 bg-slate-50 flex items-center justify-between">
          <span className="text-xs text-slate-500">
            Mẫu 04/DM • Danh mục thiết bị y tế áp dụng trong thanh toán BHYT (Loại HS 11)
          </span>
          <div className="flex items-center gap-2">
            <button
              type="button"
            ...
            <button
              type="button"
              onClick={onExportXml}
              className="px-4 py-2 bg-cyan-600 hover:bg-cyan-700 text-white rounded-xl text-xs font-bold flex items-center gap-1.5 shadow-md shadow-cyan-600/20"
            >
              <Download size={14} />
