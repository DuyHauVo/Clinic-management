...
interface NhanLucXmlModalProps {
  isOpen: boolean;
  onClose: () => void;
  items: DmNhanLucItem[];
  xmlContent: string;
      ...
      <div className="bg-white rounded-2xl max-w-4xl w-full max-h-[90vh] flex flex-col shadow-2xl overflow-hidden border border-slate-200">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h3 className="text-base font-extrabold text-slate-900">
                  Cấu Trúc XML & Chuỗi Base64 Ký Số Mẫu 02/DM
                </h3>
                <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-indigo-100 text-indigo-800 border border-indigo-200">
                  Loại HS 71 - GuiDanhMuc02_NLKCB
                </span>
              </div>
              <p className="text-xs text-slate-500 mt-0.5">
                Dữ liệu hiện hành • {items.length} nhân sự KCB
              </p>
            </div>
          </div>
          <button
        ...
        <div className="p-6 overflow-y-auto flex-1 space-y-4">
          {items.length === 0 ? (
            <div className="p-8 text-center bg-slate-50 rounded-2xl border border-dashed border-slate-300 space-y-3">
              ...
              <p className="text-sm font-bold text-slate-800">Chưa có dữ liệu Nhân Lực KCB</p>
            ...
            <>
              ...
              <div className="flex items-center gap-2 border-b border-slate-200 pb-3">
                ...
                >
                  <FileCode size={14} />
                  <span>XML Gốc (&lt;DANHSACH_DMNHANLUCKBCB&gt;)</span>
                </button>
                ...
                >
                  <Send size={14} />
                  <span>Đặc Tả API Cổng EGW (GuiDanhMuc02_NLKCB)</span>
                </button>
              </div>
              ...
              {tab === 'xml' && (
                <div className="space-y-2">
                  <div className="flex items-center justify-between text-xs text-slate-500">
                    <span>Định dạng XML chuẩn BHXH (Loại 71)</span>
                    <button
                      type="button"
              ...
              {tab === 'base64' && (
                <div className="space-y-2">
                  <div className="flex items-center justify-between text-xs text-slate-500">
                    <span>Chuỗi Base64 ký số truyền vào tham số <code className="text-indigo-600 font-mono">fileHsBase64</code></span>
                    <button
                      type="button"
                  ...
                  <div className="p-4 bg-slate-900 text-slate-200 rounded-xl space-y-3 font-mono text-[11px]">
                    <div className="flex items-center justify-between border-b border-slate-800 pb-2">
                      <span className="text-emerald-400 font-bold">
                        POST https://egw.baohiemxahoi.gov.vn/api/DanhMucGW/GuiDanhMuc02_NLKCB
                      </span>
                  ...
                  <div className="p-4 bg-emerald-50 rounded-xl border border-emerald-200 flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-emerald-700 text-[11px] mt-0.5">
                        Gói tin chứa {items.length} nhân sự sẽ được đóng gói mã hóa chuẩn Base64 gửi mô phỏng.
                      </p>
                    </div>
                    <button
                      type="button"
        ...
        <div className="p-4 border-t border-slate-200 bg-slate-50 flex items-center justify-between">
          <span className="text-xs text-slate-500">
            Mẫu 02/DM • Biểu mẫu Danh mục Nhân lực KCB BHYT
          </span>
          <div className="flex items-center gap-2">
            <button
              type="button"
            ...
            <button
              type="button"
              onClick={onExportXml}
              className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-bold flex items-center gap-1.5 shadow-md shadow-indigo-600/20"
            >
              <Download size={14} />
